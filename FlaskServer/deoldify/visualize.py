...
def get_artistic_image_colorizer(
    ...
    logging.error(os.listdir(root_folder))
...
class AzureModelImageVisualizer(ModelImageVisualizer):
    def __init__(self, filter: IFilter, results_dir: str = None):
        super().__init__(filter = filter, results_dir = results_dir)

    
    def get_transformed_image_from_url(
        self,
        url: str,
        path: str = 'test_images/image.png',
        figsize: (int, int) = (20, 20),
        ...
        input_img = self._get_image_from_url(url)
        input_img.save(path)
    ...
    def get_transformed_image_from_memory(
        self,
        image: Image,
        render_factor: int = None,
        watermarked: bool = True,
...
def get_azure_artistic_image_colorizer(
    root_folder: Path = Path('./'),
    weights_name: str = 'ColorizeArtistic_gen',
    render_factor: int = 35,
    results_dir='result_images',
    ...
    learn = gen_inference_deep(root_folder=root_folder, weights_name=weights_name)
    filtr = MasterFilter([ColorizerFilter(learn=learn)], render_factor=render_factor)
    vis = AzureModelImageVisualizer(filtr, results_dir = results_dir)
